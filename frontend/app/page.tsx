...
export default function Home() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-2">
      ...
      {/* === FLOW === */}
      <div className="bg-zinc-900 rounded-xl p-5 space-y-0">

        {/* SOURCE */}
        <div>
          <p className="text-xs text-zinc-500 mb-2 font-semibold tracking-wider uppercase">Source</p>
        ...
        {/* FILTER + KEYWORDS */}
        <div>
          <p className="text-xs text-zinc-500 mb-2 font-semibold tracking-wider uppercase">Filter</p>
        ...
        {/* OUTPUT */}
        <div>
          <p className="text-xs text-zinc-500 mb-2 font-semibold tracking-wider uppercase">Output</p>
          <div className="flex flex-wrap gap-2">
            {OUTPUTS.map((o) => (
              <span
                key={o.id}
          ...
          <div className="flex flex-wrap gap-4 mt-3 text-xs text-zinc-500">
            <a href="/api/calendar" className="text-emerald-400 hover:underline">calendar.ics ↗</a>
            <a href="/api/feed" className="text-emerald-400 hover:underline">feed.json ↗</a>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-3 pt-1">
          <button
            ...
            className="px-8 py-3 bg-emerald-700 hover:bg-emerald-600 disabled:bg-zinc-700 disabled:text-zinc-500 rounded-xl text-sm font-bold transition-all flex items-center gap-2"
          >
            {status.running ? (
              <>
          ...
          {status.lastRun && (
            <span className="text-[10px] text-zinc-600">
              last: {new Date(status.lastRun).toLocaleString("ja-JP")}
            </span>
          )}
        </div>
      </div>
      ...
      {/* agent log */}
      {agentLog.length > 0 && (
        <div className="bg-zinc-900 rounded-xl p-4 space-y-1 font-mono text-xs max-h-40 overflow-y-auto">
          {agentLog.map((line, i) => (
            <p key={i} className={line.includes("error") ? "text-red-400" : "text-zinc-400"}>{line}</p>
          ))}
        </div>
      )}
      ...
      <div className="bg-zinc-900 rounded-xl p-5 space-y-3">
        <h2 className="text-sm font-semibold text-zinc-300">Results ({events.length})</h2>
        {events.length === 0 && !status.running && (
          <p className="text-zinc-500 text-sm">Run the pipeline to see results.</p>
        )}
        {events.map((ev) => (
          <div key={ev.uid} className="border-l-2 border-emerald-800 pl-3 space-y-1">
            <a
              href={ev.url}
              target="_blank"
            ...
            <p className="text-xs text-zinc-500">
              ...
              <span> · 掲載: {ev.published}</span>
              {ev.eventDate && <span> · 実施: {ev.eventDate}</span>}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="text-center text-[10px] text-zinc-600 pb-4">
        Google Calendar で購読: calendar.ics の URL をコピー → 「他のカレンダー」→「URL で追加」
      </div>
    </div>
  );
}
